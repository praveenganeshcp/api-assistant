...
@Injectable()
export class FindEndpointByPathMatchUsecase
  implements
    Usecase<
      FindEndpointByPathMatchUsecaseInput,
      CanBeNull<FindEndpointByPathMatchUsecaseOutput>
    >
{
  ...
  async execute(
    data: FindEndpointByPathMatchUsecaseInput
  ): Promise<CanBeNull<FindEndpointByPathMatchUsecaseOutput>> {
    const applicationAllEndpoints: Endpoint[] =
      await this.endpointsRepo.findAll({
    ...
    for (const endpoint of endpoints) {
      ...
      if (result) {
        ...
        return {
          endpoint: endpoint,
          params:
            result.params as FindEndpointByPathMatchUsecaseOutput['params'],
        };
      }
    }
    return null;
