...
import { CoreEngineInvalidVariablePathException, CoreEngineUnSupportedVariableTypeException, CRUDSupportedVariablesInfo, CRUDSystemVariables, PlaceholderDataSource } from "@api-assistant/crud-engine-core";

export class VariableValueResolver {

  private readonly logger = new Logger(VariableValueResolver.name);
  private readonly variableTypeResolverService: VariableTypeResolverService = new VariableTypeResolverService();

    private resolveRequestBodyVariable(
        value: string,
        inputBody: Document
    ) {
    ...
    public resolve(value: string, inputData: PlaceholderDataSource) {
      this.logger.log(`Resolving variable value ${value} with input data`, inputData);
        const type = this.variableTypeResolverService.resolve(value);
      this.logger.log(`Resolved variable ${value} type to ${type}`);
        switch(type) {
            case 'RequestBody': {
                const result = this.resolveRequestBodyVariable(value, inputData.requestBody);
                if(this.variableTypeResolverService.resolve(result as unknown as string) === 'ObjectId') {
