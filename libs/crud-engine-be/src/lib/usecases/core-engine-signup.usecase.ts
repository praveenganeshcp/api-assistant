...
@Injectable()
export class CoreEngineSignupUsecase implements Usecase<CoreEngineCRUDUsecaseInput, CoreEngineSignupUsecaseOutput> {
    ...
    async execute(data: CoreEngineCRUDUsecaseInput): Promise<CoreEngineSignupUsecaseOutput> {
        this.logger.log('creating new user account', data);
        const response = await this.coreEngineCRUDUsecase.execute(data) as CoreEngineSignupResponse;
